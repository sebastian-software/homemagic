...
#[tokio::test]
async fn schema_v7_fixture_should_apply_operation_binding_migration() -> Result<(), BoxError> {
    let directory = tempfile::tempdir()?;
    let path = directory.path().join("schema-v7.sqlite3");
    let repository = SqliteRepository::open(&path)?;
    drop(repository);
    let connection = Connection::open(&path)?;
    connection.execute_batch(include_str!("fixtures/schema-v7.sql"))?;
    drop(connection);

    let repository = SqliteRepository::open(&path)?;
    let health = repository.health().await?;
    let connection = Connection::open(&path)?;
    let binding_tables: i64 = connection.query_row(
        "SELECT COUNT(*) FROM sqlite_master
         WHERE type = 'table' AND name = 'matter_operation_bindings'",
        [],
        |row| row.get(0),
    )?;
...
#[tokio::test]
async fn schema_v8_fixture_should_apply_fabric_staging_migration() -> Result<(), BoxError> {
    let directory = tempfile::tempdir()?;
    let path = directory.path().join("schema-v8.sqlite3");
    let repository = SqliteRepository::open(&path)?;
    drop(repository);
    let connection = Connection::open(&path)?;
    connection.execute_batch(include_str!("fixtures/schema-v8.sql"))?;
    drop(connection);

    let repository = SqliteRepository::open(&path)?;
    let health = repository.health().await?;
    let connection = Connection::open(&path)?;
    let staging_tables: i64 = connection.query_row(
        "SELECT COUNT(*) FROM sqlite_master
         WHERE type = 'table' AND name = 'matter_fabric_stages'",
        [],
        |row| row.get(0),
    )?;
