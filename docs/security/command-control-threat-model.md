...
audit storage, Shelly dispatch and confirmation, event projection, operator
...
4. The typed adapter crosses the local network to a Shelly component.
...
Revisit this model before exposing the listener off-host, adding locks/cameras,
adding cloud relays, allowing security-risk grants, changing retention, exposing
